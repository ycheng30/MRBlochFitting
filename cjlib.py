...
from matplotlib.pyplot import imshow, clim, gray, colorbar, ginput, axis, gca, axes, figure, clf, plot, legend, xticks, yticks, gcf, title, xlabel, ylabel, grid, errorbar, xlim, ylim, subplot
...
def applymask(d, mask):
	...
	mm = [scipy.ndimage.measurements.mean( d[ii], mask ) for ii in range(d.shape[0]) ]
	ss = [scipy.ndimage.measurements.standard_deviation( d[ii], mask ) for ii in range(d.shape[0]) ]
...
def createmask(d, percentileThreshold = 75 ):
	...
	hist = numpy.histogram( L, numpy.max(L) )[0]
	second_largest = nonzero( hist == numpy.sort(hist)[-2] )[0]
...
def diffgrads(filename, outputFilename=None):
	...
	doublev = numpy.vectorize(numpy.double)
...
#	ns = f['2001','1018'].value
#	type = f['0008','0008'].value
#	d = array(f.pixel_array, dtype=float)
#
#	#  Now I need to read in the RS, RI and SS to properly rescale the data
#	for ii,seq in enumerate( f['5200','9230'] ):
#		RI = float( seq['0028','9145'][0]['0028','1052'].value )
#		RS = float( seq['0028','9145'][0]['0028','1053'].value )
#		SS = float( seq['2005','140f'][0]['2005','100e'].value )
#		d[ii] = (d[ii]*RS+RI) / ( RS*SS )
#
#	d = d.reshape( (ns, d.shape[0]/ns, d.shape[1], d.shape[2]) ).transpose((1,0,2,3)) 
#
#	return d
...
def readdicom(filename):
        ...
        blah =  r_[ f['5200','9230'][-1]['0020', '9111'][0]['0020', '9157'].value ]

        d = zeros(( r_[ blah, f.pixel_array.shape[1:]] ))
        ...
        for ii,seq in enumerate( f['5200','9230'] ):
                ...
                ind =  r_[ seq['0020', '9111'][0]['0020', '9157'].value ] - 1
                ind[ ind == -1 ] = 0
                ind =  tuple( ind )
                d[ind] = (f.pixel_array[ii]*RS+RI) / ( RS*SS )
		
        return d


def readdicom_header(filename):
	return dicom.read_file( filename )

def readbruker( filename ):

	filename_method = filename + '/method'
	filename_data = filename + '/pdata/1/2dseq'

	#  Read in the method stuff
	fp = open( filename_method )
	method = fp.readlines()
	fp.close()

	# Rows and columns
	index = [i for i,item in enumerate(method) if re.search('^##\$PVM_Matrix', item) ][0]
	tt = method[index+1].split()
	...
	if len( rcs ) == 2:
		...
		nrows,ncols,nslices = rcs

	# Rows and columns
	index = [i for i,item in enumerate(method) if re.search('^##\$MT_Offsets', item) ][0]
	index = index + 1
	tt = '' 
	while not method[index][0] == '#' and not method[index][0] == '$':
		tt = tt + ' ' + method[index] 
		index = index + 1

	freq = map( int, tt.split() )

	d = readraw(filename_data, r_[len(freq), nslices, nrows, ncols], intype='int16')
...
def readrec(filename, byteswap=False, sq=True, outfmt='float32', magnitudeOnly=False):
	...
	types_unique = numpy.unique( types )
	...
	if outfmt == 'float32':
		...
		for ii in range(ndynamics*norients*nechoes*ntypes*zsize):
			...
			d[ti,gorienti, dynamics[ii]-1,echoes[ii]-1,slices[ii]-1] = array( (numpy.fromfile(fp, dtype=numpy.dtype('uint16'), count=xsize*ysize) * rs[ii] + ri[ii] ) / (rs[ii] * ss[ii] ), outfmt)
	else:
		for ii in range(ndynamics*norients*nechoes*ntypes*zsize):
			if not gorient_prev == gorients[ii]:
				gorienti = gorienti + 1
				gorient_prev = gorients[ii]
			ti = nonzero( types_unique == types[ii] )[0]
			d[ti,gorienti,dynamics[ii]-1, echoes[ii]-1, slices[ii]-1] = numpy.fromfile(fp, dtype='uint16', count=xsize*ysize)
...
def mimage(d, cmap=None):
	imshow( d, cmap, origin='upper' )
	if cmap == None:
		gray()
	...
	xticks([])
	yticks([])
	gca().get_axes().set_position([0,0,1,1])
...
def mmontage( d, dims=None, cmap=None ):
...
def mmontagecb( d, dims=None, cmap=None ):
	mmontage(d, dims, cmap)
	ax = gcf().add_axes([0.85, 0.05, 0.04, 0.9])
...
def t2fit(te, mm):
	...
	plsq = fmin_l_bfgs_b(residuals, p0, args=(te, mm), bounds=[(0, pd_max), (0.1, 200), (0, noise_max) ], approx_grad=True)
...
def ave_kern(size, sizey=None, sizez=None):
	...
	sizey = size
     ...
     if( sizez == None ):
	     A = numpy.ones( (size, sizey) )
     else:
	     A = numpy.ones( (size, sizey, sizez) )
...
def image_smooth(im, kern, N=1):
	...
	aa = im
	for ii in range(N):
		aa = numpy.numarray.nd_image.convolve( aa, kern )
	return aa

rad2deg = lambda f: f/numpy.pi*180.0
...
def calculateOffsetMap( freq, data, percentileThreshold=70, extrapolate=1.0, freqFactor=10 ):
...
def correctB0( target_freqs, freq, data, minp, percentileThreshold=70 ):
	...
	coords = array( numpy.nonzero( data[0] > thresh ) ).transpose()
...
##  Fix the asymmetry map based on the water offset calculated above
def calculateAsym( freq, data, minp, percentileThreshold=70 ):
	...
	for ii,coord in enumerate(coords):
		...
		tt = interpolate.splrep(freq[inds_pos], mm, k=3)
...
def wassrLL(freq, data, show=False):
...
#### YingCheng 07/17/2013 added plotting to see the LorFit of wassr data
	A = x[0]
	x0 = x[1]
	w = x[2]
	
	if show:
		figure(120, figsize=(14, 10), dpi=100)
		clf()

		### Interp the acquired freq, so LorFit curve is smoother
		newfreq = linspace(freq[0], freq[-1], 4*len(freq))
		lorentzian_fit = f(newfreq, A, x0, w)
		plot( freq, data*100, 'bo', label='Acquired' )
		plot( newfreq, lorentzian_fit*100, 'k-', label='Fitted' )
		xlim((freq[-1], freq[0]))
		ylim((0,100))
		grid('on')
		legend(loc='best', prop={'size':10})
		figure(110).canvas.draw()
...
def cestFitROI( freq, d, mask, fitinds, newfreq=None ):

	## Run through each of the pionts in the ROI and 
	## calculate the cestFit, then average the z-spectra

	points = nonzero( mask == 1 )

	inds_sat = nonzero( abs( freq ) < 60 )
	inds_nosat = nonzero( abs( freq ) > 60 )

	zspect_fixed = zeros(( len(inds_sat), ))
	zspect_lorentzian = zeros(( len(inds_sat), ))
	
	N = sum(mask)

	for ii in range(sum(mask)):
		r,c = points[0][ii], points[1][ii]
		
		mm = d[inds_sat,c,r] / mean( d[inds_nosat,c,r] ) * 100
		mm  = mm[0]

		if all( isfinite( mm ) ):
			newfreq, mm_fixed, lorentzian_fixed, A, x0, w, b, k = cestFit( freq[inds_sat], mm, fitinds, newfreq=freq[inds_sat] )

			zspect_fixed = zspect_fixed + mm_fixed
			zspect_lorentzian = zspect_lorentzian + lorentzian_fixed
		else:
			N = N - 1

	zspect_fixed = zspect_fixed / N
	zspect_lorentzian = zspect_lorentzian / N

	return zspect_fixed, zspect_lorentzian

def cestMapRanges( freq, zspect, range ):
		
	# Get the fit indices
	range = sort(range)
	fitinds = r_[ find( (freq >= range[0]) * (freq <= range[1]) ) ]

	return mean( zspect[fitinds], axis=0 )

def cestFitM0CorrectVolume( freq, d, mask, fitranges, show=False ):
	## Run through each of the pionts in the ROI and 
	## calculate the cestFit, then average the z-spectra
	...
	# Get number of saturated points
	inds_sat = find( abs( freq < 200 ) )

	# Create output arrays
	zspect_fixed = zeros(( r_[ len(inds_sat), d.shape[1:] ]), dtype=float32)
	zspect_lorentzian = zeros(( r_[ len(inds_sat), d.shape[1:] ]), dtype=float32)

	A = zeros(( d.shape[1:] ), dtype=float32)
	x0 = zeros(( d.shape[1:] ), dtype=float32)
	w = zeros(( d.shape[1:] ), dtype=float32)
	b = zeros(( d.shape[1:] ), dtype=float32)

	pbar = ProgressBar(widgets=['Calc zspect', Percentage(), Bar(), ETA()], maxval=N).start()
	for ii in arange(sum(mask)):
		
		# Get slice, row, col
		sl,r,c = points[0][ii],points[2][ii], points[1][ii]

		# Get data
		mm = d[:,sl,c,r]
		
		# Call fitting routine
		newfreq, mm_fixed, lorentzian_fixed, fitparams = cestFitM0Correct( freq, mm, fitranges, show )

		# Fill output data
		zspect_fixed[:,sl,c,r] = mm_fixed
		zspect_lorentzian[:,sl,c,r] = lorentzian_fixed

		A[sl,c,r] = fitparams[0]
		x0[sl,c,r] = fitparams[1]
		w[sl,c,r] = fitparams[2]
		b[sl,c,r] = fitparams[3]

		pbar.update(ii)
	pbar.finish()

	return newfreq, zspect_fixed, zspect_lorentzian, A, x0, w, b


def cestFitM0CorrectROI( freq, d, mask, fitranges, show=False ):
	## Run through each of the pionts in the ROI and 
	## calculate the cestFit, then average the z-spectra
	...
	zspect_fixed = zeros((N, len(find(freq<200))))
	zspect_lorentzian = zeros((N, len(find(freq<200))))
	...
	for ii in range(sum(mask)):
		...
		newfreq, mm_fixed, lorentzian_fixed,aa = cestFitM0Correct( freq, mm, fitranges, show )

		zspect_fixed[ii] = mm_fixed
		zspect_lorentzian[ii] = lorentzian_fixed

	return newfreq, mean( zspect_fixed, axis=0 ), std( zspect_fixed, axis=0 ), mean( zspect_lorentzian, axis=0 ), std( zspect_lorentzian, axis=0 )


##  This version assumes there are M0 (unsaturated volumes) acquired throughout the
##  acquisition.  They will then be used to correct for any drift during the scan
##
##  freq      - all the frequencies (saturated and unsaturated), they might be random order
##  mm        - all the acquired data
##  fitranges - list of ranges of frequencies to which we will fit
def cestFitM0Correct( freq, mm, fitranges, show=False ):
	...
	def lorentzian_min(x, freq, data):
		...
		#figure(101)
		#clf()
		#plot( freq, lorentzian(freq, x[0], x[1], x[2], x[3], x[4]), 'go' )
		#plot(freq, data, 'bx' )  
		#figure(101).canvas.draw()
	...
	def lorentzianFit(freq, data):
		...
		#x = curve_fit(lorentzian, freq, data, p0=(A,x0,w,b,k), sigma=None)[0]
		x = fmin_l_bfgs_b(lorentzian_min, fprime=[], x0=(A,x0,w,b,k), args=(freq,data), approx_grad=True, bounds=((90, 110),(-1, 1),(0, 2),(None, None),(1,1)))[0]
	...
	inds_nosat = find( freq > 250 )
	inds_sat = find( freq < 250 )

	## 0) Normalize the signal relative to the first unsaturated points
	mm = mm / mm[0] * 100

	## 1) Fit the M0 (unsaturated volumes) to a smooth spline
	t = arange( len (mm)  )
	tt_nosat = t[inds_nosat]
	mm_nosat = mm[inds_nosat]

	w = 1./max(mm_nosat)*ones((mm_nosat.shape))
	tck = interpolate.splrep(tt_nosat, mm_nosat, s=10., w=w)
	mm_nosat_correction = interpolate.splev( t[inds_nosat], tck)

	if show:
		figure(100)
		clf()
		subplot(3,2,1)
		plot( tt_nosat, mm_nosat, 'bo', label='Acquired' )
		plot( tt_nosat, mm_nosat_correction, 'rx', label='Fitted' )
		legend(loc='best', prop={'size':10})
		title('M0 Data')
		ylim((0,110))
		grid('on')
	...
	## Now calculate the correction for the unsaturated data
	mm_correction = interpolate.splev( t[inds_sat], tck)
	mm_corrected = mm[inds_sat] + ( 100 - mm_correction )

	if show:
		subplot(3,2,2)
		plot( t[inds_sat], mm[inds_sat], 'b', label='Acquired' )
		plot( t[inds_sat], mm_corrected, 'r', label='Corrected' )
		grid('on')
		ylim((0,110))
		title('Uncorrected / Corrected Data (Acq Order)')
		legend(loc='best', prop={'size':10})
		figure(100).canvas.draw()

	## Reorder the corrected data so the frequencies are increasing
	inds = argsort( freq[inds_sat] )
	mm_corrected = mm_corrected[inds]

	if show:
		subplot(3,2,3)
		plot( freq[inds_sat][inds], mm[inds_sat][inds], 'b', label='Acquired' )
		plot( freq[inds_sat][inds], mm_corrected, 'r', label='Corrected' )
		xlim((40,-40))
		ylim((0,110))
		grid('on')
		title('Uncorrected / Corrected Data (Freq Order)')
		legend(loc='best', prop={'size':10})
		figure(100).canvas.draw()

	##--------------------------------------------------------------------
	##  At this point we assume freq,mm are sorted and only the saturated
	##--------------------------------------------------------------------
	freq_corrected = freq[inds_sat][inds]

	## Given the fit ranges, create the fitinds
	fitinds = array([])
	for range in fitranges:
		range = sort(range)
		fitinds = r_[ fitinds, r_[ find( (freq_corrected > range[0]) * (freq_corrected < range[1]) ) ] ]
	fitinds = fitinds.astype(int16)

	## Fit the data
	A,x0,w,b,k = lorentzianFit( freq_corrected[fitinds], mm_corrected[fitinds] )

	newfreq = freq_corrected

	## Shift the data by the x0 by interpolateion - acquired data
	ff = extrap1d( interpolate.interp1d( freq_corrected-x0, mm_corrected, kind='linear') )
	mm_fixed = ff( newfreq )

	## Shift the data by the x0 by interpolateion - Lorentzian data
	gg = extrap1d( interpolate.interp1d( freq_corrected-x0, lorentzian(freq_corrected, A, x0, w, b,k), kind='linear' ) )
	lorentzian_fixed = gg( newfreq )

	if show:
		subplot(3,2,4)
		plot( freq_corrected, mm_fixed, 'b', label='Corrected' )
		plot( freq_corrected, lorentzian_fixed, 'g', label='Lorentzian' )
		xlim((2,-2))
		xlim((40,-40))
		ylim((0,110))
		grid('on')
		...
		subplot(3,2,6)
		plot( freq_corrected, lorentzian_fixed - mm_fixed, 'g' )
		plot([-40, 40], [0,0], 'k')
		xlim((40,-40))
		grid('on')
		figure(100).canvas.draw()
...
def cestFitVolume( freq, d, mask, fitranges, show=False ):
	## Run through each of the pionts in the ROI and 
	## calculate the cestFit, then average the z-spectra
	...
	for ii in arange(sum(mask)):
		...
		# Get data
		mm = d[:,sl,c,r]
		mm = mm[inds_sat] / mean(mm[inds_nosat]) * 100
		
		# Call fitting routine
		newfreq, mm_fixed, lorentzian_fixed,  At, x0t, wt, bt, kt = cestFit( freq[inds_sat], mm, fitranges, newfreq=freq[inds_sat] )
...
def cestFit( freq, mm, fitinds, newfreq=None ):
	...
	def lorentzianFit(freq, data):
		...
		#x = curve_fit(lorentzian, freq, data, p0=(A,x0,w,b,k), sigma=None)[0]
		x = fmin_l_bfgs_b(lorentzian_min, fprime=[], x0=(A,x0,w, b, k), args=(freq,data), approx_grad=True, bounds=((None, None),(None, None),(None, None),(None, None),(1,1)))[0]
	...
	## Find the frequency of the minimum of the zspetrum
	freq_at_min = freq[ argmin( mm ) ]

	## Given the fit ranges, create the fitinds
	if isinstance(fitinds[0], tuple):
		fitindstt = array([])
		for lowup in fitinds:
			lowup = sort(lowup)
			if( abs(lowup[0]) < 2 ):
				fitindstt = r_[ fitindstt, r_[ find( (freq > (lowup[0]+freq_at_min)) * (freq < (lowup[1]+freq_at_min)) ) ] ]
			else:
				fitindstt = r_[ fitindstt, r_[ find( (freq > lowup[0]) * (freq < lowup[1]) ) ] ]
		fitinds = fitindstt.astype(int16)
	...
	ff = extrap1d( interpolate.interp1d( freq-x0, mm, kind='linear') )
	mm_fixed = ff( newfreq )

	gg = extrap1d( interpolate.interp1d( freq-x0, lorentzian(freq, A, x0, w, b,k), kind='linear' ) )
	lorentzian_fixed = gg( newfreq )

	#figure(100)
	#clf()
	#plot( newfreq, mm_fixed, 'bo')
	#plot( newfreq, lorentzian_fixed, 'g-')
	#figure(100).canvas.draw()
...
def t2map_nnls( te, data, t2, percentileThreshold=65 ):
	...
	print 'create coords'
	coords = array( numpy.nonzero( data[0] > threshold ) ).transpose()
...
def diff_adc( evalues ):
	return mean(evalues, axis=0)
...
def showROI( cestfreq, d, sl ):

	inds_sat = nonzero( abs(cestfreq) < 200 )[0]
	inds_nosat = nonzero( abs(cestfreq) > 200 )[0]
	...
	mm,ss = applymask( d[:,sl], mask )
	ss = ss[inds_sat] / mean(mm[inds_nosat]) * 100 / sqrt(sum(mask))
	mm = mm[inds_sat] / mean(mm[inds_nosat]) * 100

	cc = cestfreq[inds_sat]

	fitinds = r_[ nonzero( abs(cc) >=12 )[0], nonzero( abs(cc) <= 1.0 )[0]] 

	## Fit the data
	newfreq, mm_fixed, lorentzian_fixed, A, x0, w, b, k = cestFit( cc, mm, fitinds, cc )
	print A, x0, w, b, k

	figure(2)
	clf()
	errorbar( cestfreq[inds_sat], mm_fixed, ss )
	plot( cestfreq[inds_sat], lorentzian_fixed, 'g--' )
	plot( cestfreq[inds_sat], mm_fixed, 'b.' )
	xlim((10,-10))
	ylim((0,110))
	grid('on')
	xlabel('Frequency Offset from Water (ppm)')
	...
	axes([ 0.6, 0.2, 0.3, 0.3] )
	imshow(d[0,sl])
	XY = array(XY)
	XY = concatenate( (XY, [XY[0,:]]), axis=0)
	plot(XY.transpose()[0], XY.transpose()[1], 'r-*')
	xticks([])
	yticks([])
	xlim((1,80))
	ylim((80,1))
	...
	#plot( cestfreq[inds_sat], mm_fixed, 'b-' )
	plot( cestfreq[inds_sat], lorentzian_fixed, 'g--' )
	xlim((max(cestfreq[inds_sat]),min(cestfreq[inds_sat])))
	ylim((0,110))
	grid('on')
	xlabel('Frequency Offset from Water (ppm)')
	ylabel('Relative Signal (%))')
